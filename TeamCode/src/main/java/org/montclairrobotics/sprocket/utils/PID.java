...
public class PID implements Updatable,Input<Double>{
	...
	public PID() {
		this.input = null;
		this.P = this.I = this.D = 0.0;
		this.minOut = this.maxOut = this.minIn = this.maxIn = 0.0;
	...
	private double calculate(double val) {
		...
		double out = P * error * loopTime + I * totalError + D * (-dVal) / loopTime;
